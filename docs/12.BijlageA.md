...
## Al verwerkt in schema 3.0.0 maar nog niet in het informatiemodel IMEV 3.0.1
...
Aanpassing:
De PRContour is toegevoegd aan de diagram IX-B van de MBA-activiteit “opslaan stoffen ADR-klasse 1.3”. 
PRContour is een subtype van EVContour net als het Aandachtgebied, dat al wel in het diagram getoond wordt.
Dit is alleen een visuele wijziging, want het IMEV 3.0.0 verbiedt het niet om andere subtypes te gebruiken dan die nu in het diagram getoond worden.
Hierdoor is het schema niet veranderd.
...
Aanpassing:
Er zat een fout in het formele patroon van het attribuut bronhouder van het ExterneVeiligheidsObject.
Er stond: [A-Z][a-z][-]{1,100} en dat is geworden: [A-Z,a-z, ,-]{1,100}.
...
Aanpassing:
De minimum en maximum waarden In het attribuut aantalAutobussen van objecttype TankenCNG waren omgedraaid en zijn nu dus in het informatiemodel teruggedraaid.
...
Aanpassing:
Typefout is verbeterd: Enumeratie MateriaalWindtubineMast moet zijn MateriaalWindturbineMast
Dit moest nog aangepast worden in het informatiemodel, maar was al wel verwerkt in IMEV3.0.0_schema.json.

## Nog niet verwerkt in schema 3.0.0 en niet in het informatiemodel IMEV 3.0.1

De  wijzigingsverzoeken/ bugs in deze paragraaf zijn verwerkt in IMEV 3.0.1 en waren nog niet verwerkt in IMEV3.0.0_schema en moeten dus nog verwerkt worden in het volgende schema: IMEV3.0.1_schema.json.
